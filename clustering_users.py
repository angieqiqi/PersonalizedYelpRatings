...
array_userRatings = {}
for l in open("data/filtered/user.json", "r"):
    user = json.loads(l)
    array_userRatings[user["user_id"]] = user["average_stars"]

index = 0
for user in id_user:
    array_dictionary_users[index]["userRatings"] = array_userRatings[user]
    index = index + 1
